...
const processImg = async (name: string, width: number, height: number): Promise<number | string> => {
  let img
  let data
  const fullPath: string = path.join(__dirname, '..', '..', 'Images', 'full', name + '.jpg')
  const processedPath: string = path.join(__dirname, '..', '..', 'Images', 'processed', name + String(width) + String(height) + '.jpg')
